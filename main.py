...
def collect_posture_data(output_csv='data/posture_data.csv', duration=60):
    """
    ...
    with pose:
        while cap.isOpened():
            ...
            if results.pose_landmarks:
                ...
                landmarks = results.pose_landmarks.landmark[:23]# only need up to 23 landmarks for potential hand detection later
                slouching = is_slouching_hardcoded(landmarks)

                label = 1 if slouching else 0
                features = [
                    landmark.x for landmark in landmarks[:23]
                ] + [
                    landmark.y for landmark in landmarks[:23]
                ] + [
                    landmark.z for landmark in landmarks[:23]
                ] + [
                    landmark.visibility for landmark in landmarks[:23]
                ] + [label]
    ...
    with open(output_csv, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow([f'x_{i}' for i in range(23)] + [f'y_{i}' for i in range(23)] + [f'z_{i}' for i in range(23)] + ['label'])
